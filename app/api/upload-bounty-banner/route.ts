...
export async function POST(request: Request) {
  ...
  try {
    ...
    const uploadResult = await new Promise((resolve, reject) => {
      cloudinary.uploader.upload_stream({ folder: 'futoforge_bounty_banners' }, (error, result) => {
    ...
    */
    ...
    const simulatedUrl = `https://res.cloudinary.com/${process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME}/image/upload/v1678901234/simulated_bounty_banner_${Date.now()}.png`
